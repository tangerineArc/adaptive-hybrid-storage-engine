...
private:
    ...
    void build_lmdb_index() {
        ...
        is_migrating = true; 
        std::cout << "[MIGRATION] 🔀 Phase 2: Dual-write enabled. Building index in background...\n";
        ...
        for (it->SeekToFirst(); it->Valid(); it->Next()) {
            ...
            // If the live thread already dual-wrote this key, LMDB will return MDB_KEYEXIST
            // and safely ignore this older snapshot value. First-Write-Wins!
        ...
        is_lmdb_ready = true;
        is_migrating = false;
        std::cout << "[MIGRATION] ✅ Phase 3: LMDB Index Built (" << count << " keys). Routing scans to B+Tree.\n";
    }
    ...
    void drop_lmdb_index() {
        ...
        // Empty the LMDB database (0 deletes the data, 1 would delete the DB instance entirely)
        MDB_txn* txn;
        mdb_txn_begin(lmdb_env, NULL, 0, &txn);
...
public:
    ...
    void Put(const std::string& key, const std::string& value) {
        ...
        // If we are fully ready OR actively migrating, we must mirror the write to LMDB.
        if (is_lmdb_ready || is_migrating) {
            ...
            if (is_lmdb_ready || is_migrating) { 
                ...
                // Live writes ALWAYS overwrite (0 flag), because they are the newest data
