...
private:
  ...
  // FIX: Minimum traffic volume required to care about the ratio
  const uint64_t MIN_TRAFFIC_THRESHOLD = 500;
  ...
  void log_trace(uint64_t r, uint64_t w, double ratio,
                 const std::string &state_str, bool triggered) {
    std::ofstream file("workload_trace.csv", std::ios::app);
    if (file.is_open()) {
      auto now = std::chrono::system_clock::now().time_since_epoch().count();
      file << now << "," << r << "," << w << "," << ratio << "," << state_str
           << "," << (triggered ? "YES" : "NO") << "\n";
    }
  }
  ...
  void observer_loop() {
    while (running) {
      ...
      // 🛑 THE VOLUME GATE LOGIC
      if (total > MIN_TRAFFIC_THRESHOLD) {
        ratio = static_cast<double>(current_reads) / total;
      } else {
        // Not enough traffic to matter. Force a neutral 0.5 ratio
        // so we don't accidentally trigger T_HIGH or T_LOW on background noise.
        ratio = 0.5;
      }
      ...
      if (current_state != WorkloadState::READ_HEAVY && high_count >= K) {
        ...
        // Notice we look at the IMMEDIATE ratio for T_LOW to drop out quickly
        // to save RAM
        current_state = WorkloadState::WRITE_HEAVY;
        state_str = "WRITE_HEAVY";
        std::cout << "\n[BRAIN]  REVERTING. Workload became Write-Heavy. "
                     "Dropping Index.\n";
        history.assign(W, 0.0);
      } else if (current_state == WorkloadState::STABLE && ratio < T_LOW &&
                 total > MIN_TRAFFIC_THRESHOLD) {
        // Added traffic check here too so it doesn't default to WRITE_HEAVY on
        // 0 traffic
        current_state = WorkloadState::WRITE_HEAVY;
        state_str = "WRITE_HEAVY";
      } else {
        // Maintain current state string for logging
        if (current_state == WorkloadState::READ_HEAVY)
          state_str = "READ_HEAVY";
        if (current_state == WorkloadState::WRITE_HEAVY)
          state_str = "WRITE_HEAVY";
      }

      log_trace(current_reads, current_writes, ratio, state_str,
                migration_triggered);
    }
  }

public:
  WorkloadMonitor(int window_duration_ms = 10000)
  ...
  inline void record_read() {
    read_ops.fetch_add(1, std::memory_order_relaxed);
  }
  inline void record_write() {
