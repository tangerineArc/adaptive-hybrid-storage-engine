...
// Format keys sequentially to simulate time-series data
std::string MakeKey(int i) {
  std::ostringstream oss;
  oss << "time_" << std::setw(11) << std::setfill('0') << i;
  return oss.str();
}

int main() {
  std::cout << "--- Pre-generating Sequential Data ---\n";
  ...
  // Notice: NO SHUFFLING. The data remains perfectly sorted.

  std::string payload(100, 'x'); // 100 bytes of dummy data

  // Clean up old databases
  std::filesystem::remove_all("./bench_rocks_seq");
  std::filesystem::remove_all("./bench_lmdb_seq");
  std::filesystem::create_directories("./bench_rocks_seq");
  std::filesystem::create_directories("./bench_lmdb_seq");

  // ==========================================
  // 1. ROCKSDB: Sequential Insertions
  // ==========================================
  rocksdb::DB* rocks_db;
  rocksdb::Options options;
  options.create_if_missing = true;
  rocksdb::DB::Open(options, "./bench_rocks_seq", &rocks_db);

  std::cout << "\nStarting RocksDB Inserts (" << NUM_RECORDS << " sequential keys)...\n";
  ...
  // ==========================================
  // 2. LMDB: Sequential Insertions
  // ==========================================
  MDB_env* lmdb_env;
  MDB_dbi lmdb_dbi;
  mdb_env_create(&lmdb_env);
  mdb_env_set_mapsize(lmdb_env, 2048UL * 1024UL * 1024UL); // 2GB
  mdb_env_open(lmdb_env, "./bench_lmdb_seq", 0, 0664);

  MDB_txn* txn;
  mdb_txn_begin(lmdb_env, nullptr, 0, &txn);
  mdb_dbi_open(txn, nullptr, MDB_CREATE, &lmdb_dbi);

  std::cout << "\nStarting LMDB Inserts (" << NUM_RECORDS << " sequential keys, batched " << LMDB_BATCH_SIZE << ")...\n";
  ...
  for (int i = 0; i < NUM_RECORDS; ++i) {
    ...
    // Fair fight: Standard put without the explicit MDB_APPEND flag
